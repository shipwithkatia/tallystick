...
- **The quote gate forgives typography and nothing else.** The tolerance for typographic drift was an edit budget of 2% of the span's length. It was corrected once, by removing its floor, and that correction was written up here as if the class were closed: it was not. The budget had no ceiling either, so it bought edits in proportion to length. An adversarial review of v0.8 demonstrated it: a 308-character source saying `12.4 million euro` cited as `92.4 million euro` reported `BOOKS BALANCE`, exit 0, and a 2,645-character span had its closing sentence swapped from "no adverse events in the trial" to "safe and effective for all patients" and balanced too. A ceiling would not have closed it — one edit moves a digit. The budget is gone: what a quote says (letters, digits, and separators standing inside a number) must survive character for character, while spacing and punctuation need not. `tests/test_quote_content_survives.py` holds both demonstrated cases open, and the mutation test that used to assert `14 million` may be cited as `15 million` now asserts the opposite. The lesson, since this is the second entry about the same gate: a fix aimed at the example a review gave, rather than at the rule behind it, leaves the rule broken and the write-up wrong.
