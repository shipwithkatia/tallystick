...
- [ ] v0.5.3 benchmark rerun — the traces changed (splitter, question root), so the v0.5.2 rows are not comparable sentence for sentence; the new table replaces the old one and the old rows stay in `bench/results/`
...
Pending the rerun. The traces are rebuilt by v0.5.3's `build.py` (line breaks split sentences; QA traces carry the question), so this will be a new table, not a diff on the old rows. It goes here when it exists, whatever it says, with the v0.5.2 table left above it.
