...
- **All-of over any-of when a citation covers several upstream claims.** The first version marked a claim grounded if *any* claim under the cited span was grounded. Review showed that a wider, lazier citation (the whole summary instead of one sentence) then walked straight past the invented sentence — sloppiness evaded detection. Requiring every covered claim to close is stricter and occasionally flags a fine answer, but it makes the wide citation inherit the worst thing it covers, which is the correct bookkeeping. Found while reviewing the benchmark results, not yet fixed: the proposer never posts a wide span. It snaps a quote that covers two summary claims into one credit per claim, and a claim with several credits closes if any one does — so on proposer-posted files the all-of rule is bypassed by the pipeline that feeds it, and a sentence made of one true clause and one invented clause can close on the true one. The rule is right; the pipeline undercuts it. Fix and rerun are v0.5.3, and the table below was measured with the leak in place.
...
- [ ] v0.5.3 — close the any-of leak (a quote that covers several claims must stay one all-of entry; the fix is on the proposer side, so the benchmark is rerun) and replace the table; retry a proposer call once on malformed JSON (7 failed runs cost 6 of 99 traces); sentence splitter treats numbered-list fragments as one sentence; QA traces omit the question the RAGTruth responder saw (only the passages are root artifacts), so a sentence restating the question has nothing to quote and nothing for the full-history judge to check either, since it is shown the same document artifacts — add the question as a root
...
| RAGAS faithfulness | ✅ | ❌ | ❌ | ❌ |
| LLM-as-judge, one-hop context | ✅ | ❌ *(F1 0.09 above)* | ❌ | ❌ |
| LLM-as-judge, full history | ✅ | ✅ *(F1 0.62 above)* | ❌ | ❌ |
