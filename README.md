...
- [ ] v0.7.1 run — 228 AgentHallu trajectories (115 labelled, of which 61 at a tool-result boundary; 113 clean), CodeAct runs excluded and counted; a first run at v0.7.0 was withdrawn in review (locate bug, fixed) and its false alarms sorted by cause with `bench/diagnose_agenthallu.py`
- [ ] a second retry when the proposer returns malformed or empty JSON: 7 of 228 runs were lost to it
...
### Real trajectories: AgentHallu (v0.7 — rerun in progress; the table lands here)
