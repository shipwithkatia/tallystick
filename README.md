...
- [x] v0.8.2: the quote gate stated as a property — only the setting may differ — after an external review showed the replacement still accepted a changed sign, currency mark, comparison operator or word boundary
- [x] v0.8.3: the quote gate reads a sequence of tokens — the words in order, each with whatever is stuck to a number in it, and the marks that end a sentence — with one clause for a closing mark added or dropped; one reading in `tallystick/tokens.py` serves the gate and the locate. This line then added that neither could forgive what the other refuses; that was never true, in either direction (see v0.8.3 in [bench/HISTORY.md](bench/HISTORY.md))
- [x] v0.8.4: the quote gate lists the characters that may differ — whitespace, the comma, the quotation marks and a sentence mark at an edge — and treats every other character, including the ones nobody has thought of, as content; of 3,565 punctuation and symbol characters, v0.8.3 let 140 be dropped from a quote without noticing and this lets 0. This line is true of the list and incomplete about the gate: the list applies after `tallystick/normalize.py` has read both sides, and that reading already forgives letter case, the kind of dash, the style of a quotation mark, Unicode composition and invisible characters, so `Polish` cited as `polish` passes (both lists in full under [Known limitations](#known-limitations))
