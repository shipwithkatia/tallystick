...
- [x] v0.6 — precision, from the diagnosis: word-level locate (punctuation and case forgiven, never a character), coverage claims for intermediate artifacts, self-evident credits, containment judged on words; `bench/diagnose.py` sorts a run's false flags by cause, offline
- [x] v0.6 benchmark rerun on the traces v0.5.3 built — same 290 sentences: precision 0.38 → 0.57, recall 0.65 → 0.61, F1 0.48 → 0.59; paired difference to the full-history judge −0.01 [−0.12, 0.09]; false flags 86 → 37 across the two runs
- [ ] v0.6.x — coverage for the remainder of a sentence the segmenter claimed only in part ("Revenue rose, and the CEO resigned" with a claim over the first clause): today the remainder is logged as uncredited characters, not posted as a claim
...
- [x] v0.7 — real agent trajectories: `tallystick/adapters/agenthallu.py` reads [AgentHallu](https://arxiv.org/abs/2601.06818) (693 runs of seven agent frameworks, 443 with the responsible step labelled by hand); `bench/agenthallu.py` compares `break_step_id` with the labelled step; credits proposed on demand from the final answer
- [ ] v0.7 run — 228 trajectories (115 labelled, of which 61 at a tool-result boundary; 113 clean), CodeAct runs excluded and counted — the first measurement of this audit against human step attribution, on traces nobody built for it
- [ ] bench: traces of three or more steps from LangSmith / OpenTelemetry exports, where the last hop is not verbatim by construction
- [ ] PyPI release
...
### Real trajectories: AgentHallu (v0.7, run pending)
