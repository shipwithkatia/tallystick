...
- [ ] v0.6 benchmark rerun — on the traces v0.5.3 built, so the two tables are on the same sentences (the splitter fix in `build.py` only changes traces built from now on)
